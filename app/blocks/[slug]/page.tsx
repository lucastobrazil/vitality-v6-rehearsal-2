...
export function generateStaticParams() {
  return listRegistryItems("blocks").map((name) => ({ slug: name }))
}
...
export default async function BlockPage({
  ...
  const all = listRegistryItems("blocks")
  if (!all.includes(slug)) notFound()

  const source = readItemSource("blocks", slug)
  ...
  return (
    <article className="space-y-6">
      <header>
        ...
        <p className="mt-1 text-sm text-muted-foreground">Block</p>
      </header>
